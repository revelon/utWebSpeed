...
$reports = [
    ...
    "SELECT DATE(NOW()) datum, 'velikost a pocet OK hashu interniho streamingu delsich 10 minut dle realmu' report, displayStatus, COUNT(*) pocet, ROUND(SUM(size)/1000/1000/1000) GB FROM file_hash_multimedia fhm JOIN file_hashflags USING (hashid) WHERE streamable=1 AND cdnStatus='ok' AND length>600 AND contentType='video' GROUP BY displayStatus",
    ...
    "SELECT DATE(NOW()) datum, 'pocet OK nezabanovanych hashu s nejakymi OK soubory ### do 120M' report, COUNT(DISTINCT fh.hashid) pocet FROM file_hashflags fh INNER JOIN file f ON (f.hashid=fh.hashid AND f.status='ok') WHERE cdnStatus='ok' AND banned<2 AND fh.hashid<120000000",
    "SELECT DATE(NOW()) datum, 'pocet OK nezabanovanych hashu s nejakymi OK soubory ### nad 120M' report, COUNT(DISTINCT fh.hashid) pocet FROM file_hashflags fh INNER JOIN file f ON (f.hashid=fh.hashid AND f.status='ok') WHERE cdnStatus='ok' AND banned<2 AND fh.hashid>=120000000",
    ...
    "SELECT DATE(NOW()) datum, 'pocet a status uzivatelskych presunu mezi realmy' report, WEEKOFYEAR(created) tyden, COUNT(*) pocet, status, SUM(IF(fr.from_realm_id=1,1,0)) na_PF, SUM(IF(fr.from_realm_id=2,1,0)) na_UT FROM file_realm_transfer_request fr WHERE (created BETWEEN '2019-12-15 00:00:00' AND '2021-01-01 00:00:00') GROUP BY tyden, status ORDER BY tyden",

    "SELECT DATE(NOW()) datum, 'pocet, velikost a typ utraceni kreditu po tydnech' report, WEEKOFYEAR(datum) tyden, COUNT(*) pocet, type, ROUND(SUM(kredit_kb)/1000/1000) GB FROM uzivatele_kreditminus uk WHERE (uk.datum BETWEEN '2019-12-15 00:00:00' AND '2021-01-01 00:00:00') GROUP BY tyden, type ORDER BY tyden",

    "SELECT DATE(NOW()) datum, 'pocet, velikost a contentType utraceneho kreditu po tydnech bez obnov z kose ### konec a start roku' report, WEEKOFYEAR(datum) tyden, COUNT(*) pocet, contentType, ROUND(SUM(kredit_kb)/1000/1000) GB FROM uzivatele_kreditminus uk LEFT JOIN file f ON (uk.file_id=f.id) LEFT JOIN file_hashflags fh USING (hashid) WHERE (uk.datum BETWEEN '2020-01-01 00:00:00' AND '2020-02-17 00:00:00') AND type!='trash_restore' GROUP BY tyden, contentType ORDER BY tyden",
    "SELECT DATE(NOW()) datum, 'pocet, velikost a contentType utraceneho kreditu po tydnech bez obnov z kose ### pokracovani roku' report, WEEKOFYEAR(datum) tyden, COUNT(*) pocet, contentType, ROUND(SUM(kredit_kb)/1000/1000) GB FROM uzivatele_kreditminus uk LEFT JOIN file f ON (uk.file_id=f.id) LEFT JOIN file_hashflags fh USING (hashid) WHERE (uk.datum BETWEEN '2020-02-17 00:00:00' AND '2021-01-01 00:00:00') AND type!='trash_restore' GROUP BY tyden, contentType ORDER BY tyden",
    "SELECT DATE(NOW()) datum, 'pocet, velikost a displayStatus utraceneho kreditu po tydnech bez obnov z kose ### konec a start roku' report, WEEKOFYEAR(datum) tyden, COUNT(*) pocet, displayStatus, ROUND(SUM(kredit_kb)/1000/1000) GB FROM uzivatele_kreditminus uk LEFT JOIN file f ON (uk.file_id=f.id) LEFT JOIN file_hashflags fh USING (hashid) WHERE (uk.datum BETWEEN '2020-01-01 00:00:00' AND '2021-02-17 00:00:00') AND type!='trash_restore' GROUP BY tyden, displayStatus ORDER BY tyden",
    "SELECT DATE(NOW()) datum, 'pocet, velikost a displayStatus utraceneho kreditu po tydnech bez obnov z kose ### pokracovani roku' report, WEEKOFYEAR(datum) tyden, COUNT(*) pocet, displayStatus, ROUND(SUM(kredit_kb)/1000/1000) GB FROM uzivatele_kreditminus uk LEFT JOIN file f ON (uk.file_id=f.id) LEFT JOIN file_hashflags fh USING (hashid) WHERE (uk.datum BETWEEN '2020-02-17 00:00:00' AND '2021-01-01 00:00:00') AND type!='trash_restore' GROUP BY tyden, displayStatus ORDER BY tyden",
    ...
    "SELECT DATE(NOW()) datum, 'pocet vsech nahranych souboru po tydnech' report, WEEKOFYEAR(upload_date) tyden, COUNT(*) pocet FROM file WHERE (upload_date BETWEEN '2019-12-15 00:00:00' AND '2021-01-01 00:00:00') GROUP BY tyden",
    "SELECT DATE(NOW()) datum, 'pocet a velikost nahranych jen novych hashu po tydnech, dle contentType' report, WEEKOFYEAR(created) tyden, COUNT(*) pocet, contentType, ROUND(SUM(size)/1000/1000/1000) GB FROM file_hashflags WHERE (created BETWEEN '2019-12-15 00:00:00' AND '2021-01-01 00:00:00') GROUP BY tyden, contentType ORDER BY tyden",

    "SELECT DATE(NOW()) datum, 'pocet vsech novych registraci po tydnech' report, WEEKOFYEAR(registrace) tyden, COUNT(*) pocet FROM uzivatele u WHERE (registrace BETWEEN '2019-12-15 00:00:00' AND '2021-01-01 00:00:00') GROUP BY tyden",
    "SELECT DATE(NOW()) datum, 'pocet vsech novych placenych registraci po tydnech' report, WEEKOFYEAR(paid) tyden, COUNT(*) pocet FROM uzivatele u LEFT JOIN user_credit uc ON (u.id=uc.uzivatele_id AND WEEKOFYEAR(registrace)=WEEKOFYEAR(paid) AND YEAR(registrace)=YEAR(paid) AND TYPE='payment') WHERE (registrace BETWEEN '2019-12-15 00:00:00' AND '2021-01-01 00:00:00') AND (paid BETWEEN '2019-12-15 00:00:00' AND '2021-01-01 00:00:00') GROUP BY tyden",
    
    "SELECT DATE(NOW()) datum, 'vsech novych SMS registraci po tydnech' report, WEEKOFYEAR(uc.paid) tyden, COUNT(*) pocet  FROM uzivatele u LEFT JOIN user_credit uc ON (u.id=uc.uzivatele_id AND (uc.paid BETWEEN (u.registrace - INTERVAL 1 SECOND) AND (u.registrace + INTERVAL 1 SECOND))) LEFT JOIN pricelist p ON (uc.pricelist_id = p.id) WHERE uc.status='paid' AND (u.registrace BETWEEN '2019-12-15 00:00:00' AND '2021-01-01 00:00:00') AND (uc.paid BETWEEN '2019-12-15 00:00:00' AND '2021-01-01 00:00:00') AND uc.type='payment' AND sms_keyword IS NOT NULL GROUP BY tyden",
    ...
    "SELECT DATE(NOW()) datum, 'pocet notifikaci o prihlaseni z noveho zarizeni po tydnech' report, WEEKOFYEAR(notified_at) tyden, COUNT(*) pocet_emailu, COUNT(DISTINCT user_id) ruznym_uzivatelum FROM user_login_device WHERE (notified_at BETWEEN '2019-12-15 00:00:00' AND '2021-01-01 00:00:00') GROUP BY tyden",
