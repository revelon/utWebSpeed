...
select distinct hashid, lower(hex(hash)) hsh, slug, name from file_hashflags join file_hash using (hashid) join file using (hashid) 
where contentType='image' and status='ok' and public='public' and displayStatus in ('maybe_porn', 'porn') and thumbImage!='' and banned=0 
and cdnStatus='ok' and upload_date between '2016-08-01 00:00:01' and '2016-09-01 00:00:01'
order by hashid desc limit 5000
    ...
    echo '<!DOCTYPE html><html lang="en"><head><meta charset="utf-8"/><style>
        ...
        button.in {background-color: beige;}
        </style></head><body>';
    ...
    while ($row = $result->fetch_assoc()) {
        ...
        echo "<br>{$row['name']}</a> &nbsp; <button type='button' onclick='s(this, \"https://uloz.to/!{$row['slug']}/\")'>Straight/UT</button> or 
        <button type='button' onclick='g(this, \"https://uloz.to/!{$row['slug']}/\")'>Gay/Children</button><hr>";
        $lastHashId = $row['hashid'];
    }

    echo "<hr>Latest hashid checked: {$lastHashId} <hr>";
    echo "<div class=fixed>Straight/UT: <textarea id=straight rows=5 cols=28 onfocus=select()></textarea>
          Gay/Children: <textarea id=gay rows=5 cols=28 onfocus=select()></textarea></div>
          <script>
