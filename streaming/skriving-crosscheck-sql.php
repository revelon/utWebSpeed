...
if ($result = $mysqli->query("SELECT * FROM external_stream_file WHERE status='incomplete' and created_date<'2019-05-20'")) {
    while ($row = $result->fetch_assoc()) {
        ...
        if ($response2->hasIssue || !$response2->video->conversion[0]->uri) {
            ...
            file_put_contents('./v2222-fail.sql', "update external_stream_file set status='failed', failed_date=now() where status='incomplete' and id={$row['id']};\n", FILE_APPEND);
        } else {
            $oks2[] = $row['external_file_id'];
            file_put_contents('./v2222-ok.sql', "update external_stream_file set status='ok', converted_date=now() where status='incomplete' and id={$row['id']};\n", FILE_APPEND);
        }
    }
    $result->close();
}
