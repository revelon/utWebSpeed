...
$reports = [
    ...
    "SELECT date(NOW()) datum, 'velikost a pocet OK hashu podle banu' report, IF(banned=2,'hardban','softban') typ, COUNT(*), ROUND(SUM(size)/1000/1000/1000) GB FROM file_hashflags WHERE cndStatus='ok' AND banned>0 GROUP BY typ",
    ...
    "SELECT date(NOW()) datum, 'velikost a pocet OK hashu OK externiho streamingu delsich 10 minut' report dle realmu, displayStatus, COUNT(*), ROUND(SUM(size)/1000/1000/1000/1000) TB FROM external_stream_file JOIN file_hashflags ON (file_hashid=hashid) WHERE cdnStatus='ok' AND status='ok' AND length>600 GROUP BY displayStatus",
    "SELECT date(NOW()) datum, 'velikost a pocet OK hashu bez jakychkoliv OK souboru' report, COUNT(*), ROUND(SUM(size)/1000/1000/1000) GB FROM file_hashflags fh LEFT JOIN file f ON (f.hashid=fh.hashid AND f.status='ok') WHERE cndStatus='ok' AND banned<2 AND f.id IS NULL",
    "SELECT date(NOW()) datum, 'velikost a pocet OK hashu s nejakymi OK souboru' report, COUNT(*), ROUND(SUM(size)/1000/1000/1000/1000) TB FROM file_hashflags fh JOIN file f ON (f.hashid=fh.hashid AND f.status='ok') WHERE cndStatus='ok' AND banned<2",
